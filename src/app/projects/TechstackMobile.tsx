    ...
    <div className="w-full flex flex-col items-center px-4 z-30">
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button className="bg-black/60 hover:bg-black/80 border border-brand/20 backdrop-blur-md text-brand-light text-xs tracking-wider uppercase px-5 rounded-xl shadow-lg flex items-center gap-2">
            <span>{selectedStack || "Filter Technology"}</span>
            <ChevronDown className="w-3 h-3 opacity-60" />
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent className="bg-zinc-900/95 border border-brand/15 backdrop-blur-md text-slate-200 min-w-[180px] rounded-xl p-1 shadow-2xl">
            ...
            return (
              <DropdownMenuItem
                ...
                className={`py-2 px-3 text-xs tracking-wide rounded-lg cursor-pointer transition-colors ${isSelected
                    ? "text-black bg-brand focus:bg-brand focus:text-black font-semibold"
                    : "text-slate-400 focus:bg-white/5 focus:text-brand-light"
                  }`}
